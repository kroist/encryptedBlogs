...
const factoryAddress = '0xA23EF767866Ce684849a9c6b72525F7D8C939c8C';
...
let instanceByKey = new Map<string, FhevmInstance>();
...
export const createFhevmInstance = async (contractAddress: string, account: ethers.Signer, provider: ethers.Provider) => {
  // 1. Get chain id
  if (instanceByKey.has(contractAddress)) {
    return instanceByKey.get(contractAddress);
  }
  ...
  instanceByKey.set(contractAddress, instance);
...
export async function getKeyAndCidFromBlockchainCrutch(
  ...
  const fhevmInstance = await createFhevmInstance(blogAddress, params.signer, params.provider);
  

  await generatePublicKey(blogAddress, params.signer, fhevmInstance);
...
export async function getKeyAndCidFromBlockchain(
  ...
  console.log("pub key", fhevmInstance.getPublicKey(blogAddress).publicKey);

  // return; 
  const k1: bigint = await fhevmInstance.decrypt(blogAddress, p[0]);
  const k2: bigint = await fhevmInstance.decrypt(blogAddress, p[1]);
